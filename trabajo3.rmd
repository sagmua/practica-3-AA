...
Si analizamos los datos obtenidos, podemos observar que existe un atributo, 'row.names', perteneciente a nuestro dataset de 'South African Heart Disease' que actua como clave primaria, es decir, como identificador, por tanto, dicho atributo nos es inutil para el aprendizaje de nuestro modelo lineal, por tanto, lo suprimimos:
...
```{r}
sudafrica_frame_3 = data.frame(sudafrica_train[,which(names(sudafrica_train) == "chd" | names(sudafrica_train) == "age" | names(sudafrica_train) == "famhist" | names(sudafrica_train) == "ldl")])
