...
```{r errorEtiquetas}
calculoErrorMatrizConfusion  = function (modelo, test){
  
  prob_test = predict(modelo, test[,-ncol(test)], type="response")
  
  pred_test = rep(0, length(prob_test))
   # predicciones por defecto 0
  pred_test[prob_test >=0.5] = 1
   # >= 0.5 clase 1
  matriz_conf = table(pred_test, test[,ncol(test)])
  print(matriz_conf)
  
  eout = mean(pred_test != test[,ncol(test)])
}
```

```{r}
m1_sudafrica = lm(chd ~ age, data=sudafrica_train)
eout_m1sud = calculoErrorMatrizConfusion(m1_sudafrica, sudafrica_test)
eout_m1sud
```
...
eout_m2sud = calculoErrorMatrizConfusion(m2_sudafrica, sudafrica_test)
...
eout_m3sud = calculoErrorMatrizConfusion(m3_sudafrica, sudafrica_test)
...
eout_m4sud = calculoErrorMatrizConfusion(m4_sudafrica, sudafrica_test)
